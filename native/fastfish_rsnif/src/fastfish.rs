...
pub fn sample(
    ...
    while active_list.len() != 0 && placed_items.len() < positions as usize {
        ...
        for _k in 0..k_samples {
            let point = crate::utils::generate_point(base_point, distance);
            if crate::utils::is_out_of_bounds(point.x, point.y, width, height) || too_close_to_someone(point.x, point.y, width, height, &grid, &placed_items, distance) {
            } else {
                found = Some(point);
                break;
            }
